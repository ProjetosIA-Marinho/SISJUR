...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const addTask = async (task: Task) => {
    ...
    if (import.meta.env.VITE_SUPABASE_URL && import.meta.env.VITE_SUPABASE_ANON_KEY) {
      try {
        const dbTask = mapTaskToDb(taskToAdd);
        if (!dbTask.assignee_id && currentUser?.id && checkUUID(currentUser.id)) {
          dbTask.assignee_id = currentUser.id;
        }
    ...
    if (!localTasks.some(t => t.id === taskToAdd.id)) {
